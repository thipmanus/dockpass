...
- Email sending is optional and only runs when `ENABLE_EMAIL_NOTIFICATIONS=true` and `RESEND_API_KEY` are configured.
...
When `ENABLE_EMAIL_NOTIFICATIONS=true` and `RESEND_API_KEY` are configured, DockPass sends one notification email to each assigned email after an admin creates a round.
...
- If email notifications are disabled or `RESEND_API_KEY` is missing, DockPass skips sending and still returns a copyable notification preview
...
Email sending is optional. Without `ENABLE_EMAIL_NOTIFICATIONS=true` and `RESEND_API_KEY`, admins can still copy the notification preview after creating a round.
...
`ENABLE_EMAIL_NOTIFICATIONS`, `RESEND_API_KEY`, and `RESEND_FROM_EMAIL` are optional. Email sending is skipped unless `ENABLE_EMAIL_NOTIFICATIONS=true` and `RESEND_API_KEY` are both set.
